#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <cuda.h>
#include <assert.h>
#include <sys/time.h>
#include "hist-equ.h"
#define checkCudaErrors(ans) {                                \
          cudaError_t err = ans;                                   \
          if (err != cudaSuccess) {                                 \
            printf("CUDA error at %s %d: %s\n", __FILE__, __LINE__, \
                  cudaGetErrorString(err)); \
            cudaDeviceReset();                                      \
            exit(EXIT_FAILURE);                                     \
          }                                                          \
        }                                                         \

void run_cpu_gray_test(PGM_IMG img_in, char *out_filename);
double exec_time;

int main(int argc, char *argv[]){
    PGM_IMG img_ibuf_g;

	if (argc != 3) {
		printf("Run with input file name and output file name as arguments\n");
		exit(1);
	}
	
    printf("Running contrast enhancement for gray-scale images.\n");
    img_ibuf_g = read_pgm(argv[1]);
    run_cpu_gray_test(img_ibuf_g, argv[2]);
    free_pgm(img_ibuf_g);
    printf("GPU total execution time: %10.8f\n", exec_time);

	return 0;
}



void run_cpu_gray_test(PGM_IMG img_in, char *out_filename)
{
    PGM_IMG img_obuf;
   
    clock_t start = clock();
    
    printf("Starting GPU processing...\n");
    img_obuf = contrast_enhancement_g(img_in);

    clock_t end = clock();
    double cpuElapsedTime =  (double(end- start) ) / CLOCKS_PER_SEC;
    exec_time += cpuElapsedTime;

    write_pgm(img_obuf, out_filename);
    free_pgm(img_obuf);
}


PGM_IMG read_pgm(const char * path){
    FILE * in_file;
    char sbuf[256];
    
    
    PGM_IMG result;
    int v_max;//, i;
    in_file = fopen(path, "r");
    if (in_file == NULL){
        printf("Input file not found!\n");
        exit(1);
    }
    
    assert(fscanf(in_file, "%s", sbuf)); /*Skip the magic number*/
    assert(fscanf(in_file, "%d",&result.w));
    assert(fscanf(in_file, "%d",&result.h));
    assert(fscanf(in_file, "%d\n",&v_max));
    printf("Image size: %d x %d\n", result.w, result.h);
    
  
    clock_t start = clock();

    //result.img = (unsigned char *)malloc(result.w * result.h * sizeof(unsigned char));
    checkCudaErrors(cudaHostAlloc(&(result.img), result.w*result.h*sizeof(unsigned char),0));
    
    clock_t end = clock();
    double cpuElapsedTime =  (double(end- start) ) / CLOCKS_PER_SEC;
    exec_time += cpuElapsedTime;

    assert(fread(result.img,sizeof(unsigned char), result.w*result.h, in_file));    
    fclose(in_file);
    
    return result;
}

void write_pgm(PGM_IMG img, const char * path){
    FILE * out_file;
    out_file = fopen(path, "wb");
    fprintf(out_file, "P5\n");
    fprintf(out_file, "%d %d\n255\n",img.w, img.h);
    fwrite(img.img,sizeof(unsigned char), img.w*img.h, out_file);
    fclose(out_file);
}

void free_pgm(PGM_IMG img)
{
    cudaFreeHost(img.img);
    //free(img.img);
}

