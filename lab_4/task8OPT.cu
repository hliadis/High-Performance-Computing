...
int main(int argc, char **argv) {
    ...
    *h_OutputGPU;
    ...
    if(h_Filter == NULL || h_Input == NULL || h_Buffer == NULL || h_OutputCPU == NULL || h_OutputGPU == NULL || h_temp == NULL){
    ...
    checkCudaErrors( cudaMalloc((void**)&d_Input,pad_imageH * pad_imageW * sizeof(double)) );
    checkCudaErrors( cudaMalloc((void**)&d_Buffer,pad_imageH * pad_imageW * sizeof(double)) );
    checkCudaErrors( cudaMemset(d_Buffer, 0, pad_imageH*pad_imageW*sizeof(double)) );
    checkCudaErrors( cudaMalloc((void**)&d_OutputGPU,imageH * imageW * sizeof(double)) );
    ...
    checkCudaErrors( cudaMemcpyToSymbol(d_Filter, h_Filter,(2 * filter_radius + 1) * sizeof(double),0,cudaMemcpyHostToDevice) );
    checkCudaErrors( cudaMemcpy(d_Input, h_temp,pad_imageH * pad_imageW * sizeof(double),cudaMemcpyHostToDevice) );

    convolutionRowGPU<<<grid,block,(block.x + 2*filter_radius)*block.y*sizeof(double)>>>(d_Buffer,d_Input,pad_imageW, filter_radius);
    
    cudaDeviceSynchronize();
    
    convolutionColumnGPU<<<grid,block,(block.y + 2*filter_radius)*block.x*sizeof(double)>>>(d_OutputGPU,d_Buffer,pad_imageH, filter_radius);
