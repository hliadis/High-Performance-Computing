...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		...
		for (j=1 , k = 1; k <=((SIZE-2)/2);k++, j+=2 ) {
			 ...
			 * of the derivative.								  */
			a1 += input[s1 + j -1] * horiz_operator[0][0];
			a1 += input[s1 + j ] * horiz_operator[0][1];
			a1 += input[s1 + j + 1] * horiz_operator[0][2];
			a1 += input[s2+ j + -1] * horiz_operator[1][0];
			a1 += input[s2+ j ] * horiz_operator[1][1];
			a1 += input[s2+ j + 1] * horiz_operator[1][2];
			a1 += input[s3 + j -1] * horiz_operator[2][0];
			a1 += input[s3 + j ] * horiz_operator[2][1];
			a1 += input[s3 + j + 1] * horiz_operator[2][2];
			
									
			a2 += input[s1 + j -1] * vert_operator[0][0];
			a2 += input[s1 + j ] * vert_operator[0][1];
			a2 += input[s1 + j + 1] * vert_operator[0][2];
			a2 += input[s2+ j -1] * vert_operator[1][0];
			a2 += input[s2+ j ] * vert_operator[1][1];
			a2 += input[s2+ j + 1] * vert_operator[1][2];
			a2 += input[s3 + j  -1] * vert_operator[2][0];
			a2 += input[s3 + j ] * vert_operator[2][1];
			a2 += input[s3 + j + 1] * vert_operator[2][2];
			...
			/* Apply the sobel filter and calculate the magnitude *
			 * of the derivative.								  */
			a1 += input[s1 + j] * horiz_operator[0][0];
			a1 += input[s1 + j+1 ] * horiz_operator[0][1];
			a1 += input[s1 + j + 2] * horiz_operator[0][2];
			a1 += input[s2+ j] * horiz_operator[1][0];
			a1 += input[s2+ j +1] * horiz_operator[1][1];
			a1 += input[s2+ j +2] * horiz_operator[1][2];
			a1 += input[s3 + j] * horiz_operator[2][0];
			a1 += input[s3 + j + 1 ] * horiz_operator[2][1];
			a1 += input[s3 + j + 2] * horiz_operator[2][2];
			
									
			a2 += input[s1 + j] * vert_operator[0][0];
			a2 += input[s1 + j +1] * vert_operator[0][1];
			a2 += input[s1 + j + 2] * vert_operator[0][2];
			a2 += input[s2+ j] * vert_operator[1][0];
			a2 += input[s2+ j +1] * vert_operator[1][1];
			a2 += input[s2+ j + 2] * vert_operator[1][2];
			a2 += input[s3 + j ] * vert_operator[2][0];
			a2 += input[s3 + j +1] * vert_operator[2][1];
			a2 += input[s3 + j + 2] * vert_operator[2][2];
			...
			if (res > 255)
				...
				output[s2+ j + 1] = (unsigned char)res;
